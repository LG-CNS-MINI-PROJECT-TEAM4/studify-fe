...
export default function WritePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        // 백엔드 컨트롤러: GET /studify/api/v1/post/detail/{postId}
  ...
  const submit = async (e) => {
    ...
    try {
      setSaving(true);
      if (isEdit) {
        // 백엔드 컨트롤러: PUT /studify/api/v1/post/{postId}
        await api.put(`/studify/api/v1/post/${id}`, payload);
        nav(`/posts/${id}`);
      } else {
        // 백엔드 컨트롤러: POST /studify/api/v1/post/posts
        const { data } = await api.post(`/studify/api/v1/post/posts`, payload);
